...
@app.route('/')
def index(): return "Бот Вкусомер запущен"

@dp.message(Command("start"))
async def cmd_start(message: types.Message, state: FSMContext):
    await state.clear()
    welcome = (
        "Здравствуйте! Я ваш персональный помощник по планированию питания Вкусомер. 😊\n\n"
        "Сегодня я помогу вам составить подробный рацион на ближайшие семь дней. В этой версии программы я использую нашу базу проверенных рецептов, чтобы вы могли начать правильно питаться уже сегодня.\n\n"
        "Вот что мы сделаем шаг за шагом:\n"
        "1. Мы определим ваши параметры и рассчитаем точную норму калорий.\n"
        "2. Я подберу меню на неделю, которое не будет повторяться каждый день.\n"
        "3. Вы получите полный список покупок, в котором я сам объединю все продукты (например, посчитаю общее количество яиц или мяса).\\n\n"
        "Важное примечание: если вы захотите больше возможностей, например, чтобы я узнавал еду по одной фотографии вашей тарелки или общался с вами как живой диетолог, вы сможете попробовать нашу Плюс-версию. Ссылку на нее я пришлю в конце нашего теста.\n\n"
        "Давайте начнем! Скажите, пожалуйста, какой у вас пол? 👤"
    )
...
async def proc_goal(message: types.Message, state: FSMContext):
    ...
    if message.text in ["Похудеть", "Набрать массу"]:
        await message.answer("Отличная цель! К какому весу в идеале вы стремитесь? Напишите, пожалуйста, цифру в килограммах: 🏁", reply_markup=ReplyKeyboardRemove())
        ...
        await message.answer("Укажите ваш уровень физической активности в течение дня: 🏃‍♂️", reply_markup=kb)
...
async def proc_tw(message: types.Message, state: FSMContext):
    ...
    kb = ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="Сидячий"), KeyboardButton(text="Средний"), KeyboardButton(text="Высокий")]], resize_keyboard=True)
    await message.answer("Записал. Теперь выберите уровень вашей активности: 🏃‍♂️", reply_markup=kb)
    await state.set_state(Survey.activity)
...
async def proc_age(message: types.Message, state: FSMContext):
    ...
    await message.answer("Напишите ваш точный рост в сантиметрах: 📏")
...
async def proc_w(message: types.Message, state: FSMContext):
    ...
    await message.answer("Есть ли у вас пищевые аллергии или продукты, которые вы исключаете из рациона? Отметьте их или нажмите Готово: ⚠️", reply_markup=kb)
...
async def proc_allg(call: types.CallbackQuery, state: FSMContext):
    if call.data == "allg_done":
        ...
        await call.message.answer("Поздравляю! Вы успешно прошли тест, и ваш личный профиль создан. ✅")
        
        res_text = (
            f"Ваша индивидуальная норма калорий для достижения цели: {norma} ккал в день. "
            "Я подготовил для вас оптимальное меню на целую неделю из нашей базы рецептов.\n\n"
            "Помните, что в нашей Плюс-версии расчеты ведет Искусственный Интеллект, который умеет "
            "распознавать еду по фото и общаться с вами как настоящий диетолог!\n\n"
...
async def show_day(call: types.CallbackQuery, state: FSMContext):
    ...
    msg = f"Ваше меню на день {day_num}. Цель на сегодня — {data['norma']} ккал.\n\n"
    btns = []
    for idx, m in enumerate(day_data['meals']):
        msg += f"Блюдо: {m['name']} ({m['calories']} ккал)\n"
        btns.append([InlineKeyboardButton(text=f"Как приготовить: {m['name']}", callback_data=f"rec_{day_num}_{idx}")])
...
async def shop_7(call: types.CallbackQuery, state: FSMContext):
    ...
    txt = "Ваш список покупок на неделю. Я уже объединил и сложил все продукты за вас:\n\n"
    ...
    txt += f"\n📢 Наш канал: {TG_CHANNEL}\n\n💎 Для доступа к ИИ-Диетологу и расчету по фото переходите в Плюс-версию: {PLUS_BOT}"
