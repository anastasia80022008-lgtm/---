...
# 1. Ваш токен. На сервере он будет браться из переменных окружения для безопасности.
#    Для локального теста используется тот, что вы предоставили.
TOKEN = os.environ.get('TOKEN', "8585043014:AAG1dnEgTV65np--Bt0rAA9Wc64LiBta9FA")

# 2. Ваша ссылка на Telegram-канал.
...
try:
    ...
    logging.error("Файл recipes.json не найден! Бот не сможет подбирать меню.")
    ALL_RECIPES = []
