...
async def cmd_start(message: types.Message, state: FSMContext):
    ...
    welcome = (
        ...
        "Я очень рад, что вы решили сделать важный шаг к своему здоровью и идеальному телу. Сегодня я помогу вам составить подробный рацион питания на ближайшие семь дней.\n\n"
        "В этой версии программы я использую нашу базу проверенных рецептов, которые идеально сбалансированы по калориям и очень просты в приготовлении.\n\n"
        "<b>Вот наш план действий:</b>\n"
        "1. Мы определим ваши текущие параметры и рассчитаем точную норму калорий специально для вас.\n"
        "2. Я подберу индивидуальное меню на неделю, которое будет соответствовать вашим целям и уровню активности.\n"
        ...
        "Если в будущем вам захочется еще больше возможностей, например, чтобы я распознавал еду по одной фотографии или общался с вами как живой диетолог, вы всегда сможете перейти в нашу Плюс-версию. Ссылку на нее я пришлю в конце нашего теста.\n\n"
...
async def proc_goal(message: types.Message, state: FSMContext):
    ...
    if message.text in ["Похудеть", "Набрать массу"]:
        await message.answer("Это отличная и вдохновляющая цель! К какому весу в идеале вы стремитесь? Напишите, пожалуйста, только число в килограммах: 🏁", reply_markup=ReplyKeyboardRemove())
...
async def proc_act(message: types.Message, state: FSMContext):
    ...
    await message.answer("Укажите ваш полный возраст (количество лет). Напишите только число: 🎂", reply_markup=ReplyKeyboardRemove())
...
async def proc_w(message: types.Message, state: FSMContext):
    ...
    await message.answer("Есть ли у вас пищевые аллергии или продукты, которые вы полностью исключаете из своего рациона? Отметьте их или нажмите Готово: ⚠️", reply_markup=kb)
...
async def proc_allg(call: types.CallbackQuery, state: FSMContext):
    if call.data == "allg_done":
        ...
        res_text = (
            f"Ваша индивидуальная норма для достижения цели составляет <b>{norma} ккал</b> в день. "
            "Я уже подготовил для вас оптимальное меню на целую неделю из нашей базы рецептов.\n\n"
            "Напоминаю, что в Плюс-версии нашего бота (ссылка ниже) расчеты ведет Искусственный Интеллект, который видит вашу еду даже по фото!\n\n"
...
async def shop_7(call: types.CallbackQuery, state: FSMContext):
    ...
    txt = "<b>Ваш сводный список покупок на неделю.</b> Я уже объединил и сложил все продукты за вас:\n\n"
