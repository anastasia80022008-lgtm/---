...
try:
    if os.path.exists('recipes.json'):
        ...
        logging.info(f"✅ ЗАГРУЖЕНО РЕЦЕПТОВ: {len(ALL_RECIPES)}")
    else:
...
async def calculate_7(callback: types.CallbackQuery, state: FSMContext):
    ...
    cals = bmr * {"Сидячий образ жизни": 1.2, "Средняя активность": 1.55, "Высокая активность": 1.725}.get(data['activity'], 1.2)
    ...
    if not plan: return await callback.message.answer("Рецепты скоро будут!")

    await state.update_data(plan=plan, target_calories=int(cals))
    buttons = [[InlineKeyboardButton(text=f"День {i}", callback_data=f"v_day_{i}")] for i in range(1, 8)]
    buttons.append([InlineKeyboardButton(text="🛒 Список продуктов на 7 дней", callback_data="shop_7")])
    await callback.message.edit_text(f"✅ Норма: {int(cals)} ккал. План (Блок {block}) готов!", reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons))
...
async def show_rec(callback: types.CallbackQuery, state: FSMContext):
    ...
    kb = [[InlineKeyboardButton(text="⬅️ Назад", callback_data=f"v_day_{d_n}")]]
    await callback.message.edit_text(txt, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(inline_keyboard=kb))
